...
class NaiveBayes:
    ...
    def train(self, positive_words, negative_words):

        print("Preparing data for learning")

        all_text_length = self.get_all_text_length(positive_words, negative_words)

        self.negative_probability = self.getRatio(negative_words, all_text_length)

        self.positive_probability = self.getRatio(positive_words, all_text_length)
    ...
    def get_all_text_length(self, set1, set2):
        return len(set1) + len(set2)


    def getRatio(self, count, total_count):
        return len(count) / total_count